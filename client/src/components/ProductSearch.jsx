...
export const ProductSearch = () => {
    ...
    const filters = ['All', 'Biscuits', 'Spreads', 'Milk', 'Snacks'];
    ...
    return (
        <div className="min-h-screen bg-gray-50 text-gray-800 font-sans pb-20">
                        ...
                        >
                            {users.map(user => (
                                <option key={user.id} value={user.id}>
                                    {user.username.charAt(0).toUpperCase() + user.username.slice(1)} ({user.preferences?.diet})
                                </option>
                            ))}
                        </select>
                        <button
                ...
                <div className="max-w-md mx-auto px-4 pb-4">
                    ...
                    <form onSubmit={handleSearch} className="relative flex items-center gap-2">
                        <div className="relative flex-1">
                            <input
                                type="text"
                                placeholder="Search organic small businesses (e.g. oat milk, nut butter)..."
                                className="w-full pl-10 pr-4 py-3 bg-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 transition-all text-sm"
                                value={query}
            ...
            {/* Product List */}
            <main className="w-full max-w-6xl mx-auto px-4 py-6 md:px-8 space-y-4">
                {aiMessage && (
                    <div className="bg-purple-50 border border-purple-100 p-4 rounded-2xl mb-4 animate-in fade-in slide-in-from-top-2">
                        <div className="flex gap-2 items-start">
                            ...
                            <div className="text-sm text-purple-800 font-medium leading-relaxed ai-summary-content" dangerouslySetInnerHTML={{ __html: aiMessage }} />
                        </div>
                    </div>
                )}
                ...
                {loading ? (
                    ...
                    <div className="text-center py-10 opacity-60">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {['Biscuits', 'Spreads', 'Milk', 'Snacks'].map(cat => (
                                <button
                                    key={cat}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 place-items-start w-full">
            ...
            {/* Fixed Right Side Panel for Score Breakdown */}
            {expandedProduct && (
                <>
                    <div
                    ...
                    />
                    <div className="fixed top-0 right-0 w-full md:w-96 h-full bg-white shadow-2xl z-40 overflow-y-auto transform transition-transform duration-300 animate-in slide-in-from-right">
                        <div className="p-6">
                            <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
                                <h4 className="font-bold text-gray-800 text-lg">Score Breakdown</h4>
                                <button onClick={() => setExpandedProduct(null)} className="text-gray-400 hover:text-gray-800 p-2 rounded-full hover:bg-gray-100 transition-colors">✕</button>
                            </div>

                            {!breakdownData ? (
                                <div className="text-center py-10 text-gray-400 animate-pulse">Loading breakdown...</div>
                            ) : (
                                <div className="space-y-4">
                                    {breakdownData.user_context?.health && (
                                        <div className="bg-purple-50 p-4 rounded-xl border border-purple-100 flex items-start gap-3">
                                            ...
                                            <p className="text-sm font-medium text-purple-800 leading-relaxed">
                                                Tailored for <span className="font-bold">{breakdownData.user_context.health}</span>
                                            </p>
                                        </div>
                                    )}
                                            ...
                                            <div key={idx} className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                                                <div className="flex justify-between items-center mb-2">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="font-bold text-gray-800 capitalize">{item.metric}</span>
                                                    </div>
                                                    <span className={`text-xs font-bold px-2 py-1 rounded-md ${item.rating === 'excellent' ? 'bg-green-100 text-green-700' :
                                                ...
                                                <p className="text-sm text-gray-600 leading-relaxed">{item.nudge}</p>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="mt-6 pt-4 border-t border-gray-100">
                                        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                                            <h5 className="font-bold text-blue-900 text-xs uppercase mb-1">Calculation Method</h5>
                                            <p className="text-sm font-bold text-blue-800 mb-2">{breakdownData.formula}</p>
                                            <p className="text-xs text-blue-700 leading-relaxed">
                                                The <strong>Health Score</strong> follows European Nutri-Score principles, heavily penalizing sugar, salt, and additives, while rewarding protein and fiber. The <strong>Value Score</strong> indicates the overall nutritional yield relative to the product's retail price.
                                            </p>
                                        </div>
                                        {breakdownData.category_rank && (
                                            <div className="mt-3 text-center text-sm font-bold text-gray-500">
                                                🏆 Ranked #{breakdownData.category_rank.rank} of {breakdownData.category_rank.total} in {breakdownData.category_rank.category}
                                            </div>
                                        )}
                                    </div>
                                </div>
            ...
            {/* Fixed Right Side Panel for Swap Suggestion */}
            {swapSuggestion && (
                <>
                    <div
                        className="fixed inset-0 bg-black/20 z-30 md:hidden"
                    ...
                    <div className="fixed top-0 right-0 w-full md:w-96 h-full bg-white shadow-2xl z-40 overflow-y-auto transform transition-transform duration-300 animate-in slide-in-from-right">
                        <div className="p-6">
                            <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
                                ...
                                <button onClick={() => setSwapSuggestion(null)} className="text-gray-400 hover:text-gray-800 p-2 rounded-full hover:bg-gray-100 transition-colors">✕</button>
                            </div>

                            <p className="text-sm text-gray-500 mb-6 px-2">{swapSuggestion.reason_main}</p>

                            <div className="flex items-center gap-4 mb-8 relative">
                                {/* Better Product */}
                                <div className="flex-1 text-center bg-green-50 p-4 rounded-xl border border-green-100 relative shadow-sm">
                                    ...
                                    <img src={swapSuggestion.better.image} className="h-20 mx-auto mb-3 object-contain mix-blend-multiply" />
                                    <div className="font-bold text-sm truncate">{swapSuggestion.better.brand}</div>
                                    <div className="text-green-700 font-extrabold text-xl">{swapSuggestion.better.score}</div>
                                </div>

                                <div className="text-gray-300 font-bold px-1">vs</div>

                                {/* Original Product */}
                                <div className="flex-1 text-center opacity-70 grayscale bg-gray-50 p-4 rounded-xl border border-gray-100">
                                    <img src={swapSuggestion.original.image} className="h-20 mx-auto mb-3 object-contain mix-blend-multiply" />
                            ...
                            {/* Comparison Reasons */}
                            <div className="bg-green-50/50 rounded-xl p-4 mb-8 border border-green-100/50">
                                <h4 className="text-xs font-bold text-green-800 uppercase tracking-wider mb-3">Why it's better:</h4>
                            ...
                            <button
                                ...
                                className="w-full py-4 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl shadow-lg shadow-green-200/50 transition-all active:scale-[0.98]"
                            >
